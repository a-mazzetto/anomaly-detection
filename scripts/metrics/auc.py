...
# %% Try to calculate AUC-proxy loss
import torch
...
def roc_auc_loss(labels, logits, weights=1.0):
    ...
    if isinstance(weights, (float, int)):
        weights = weights * np.ones_like(labels)
    assert labels.shape == weights.shape
    original_shape = labels.shape
    labels = torch.tensor(labels.flatten(), dtype=torch.float)
    logits = torch.tensor(logits.flatten(), dtype=torch.float)
    weights = torch.tensor(weights.flatten(), dtype=torch.float)
